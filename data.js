export default [
  {
    id: "69126c24-588e-5aa4-8698-bb1ac5cd6e7f",
    name: "Lina Hampton",
    verified: "afen",
    description:
      "Qui veniam labore ea irure ex. Irure velit tempor et culpa sint mollit magna sunt ea duis eiusmod. Ea anim magna occaecat dolor deserunt mollit.Consequat deserunt do sit eiusmod fugiat deserunt mollit culpa minim. Veniam mollit sint excepteur qui eu aute dolor non Lorem enim. Commodo tempor dolor tempor enim laborum et sint dolor ullamco ipsum officia voluptate laborum.",
    governmentAuthority: "Ekiti, Nigeria",
    hightestBid: 2.9,
    price: 2.37,
    auction: true,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Nisi sint aliqua ex sint sunt veniam veniam. Anim tempor qui quis irure. Fugiat amet velit tempor anim.",
      profileImage:
        "https://media.giphy.com/media/vwjfBggP3Q5vEdTx9H/giphy.gif",
    },
    created: "8/3/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1563995103864-d87d3c1fdd39?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTB8fGFmcmljYW4lMjBhcnR8ZW58MHx8MHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Victoria Montgomery",
        bidAmount: 2.9,
      },
      {
        name: "Eugene Wong",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "f0d75833-37fa-5f55-b4ee-3ac13126fe95",
    name: "Bruce Mathis",
    verified: "afen",
    description:
      "Culpa aliqua irure cillum aliqua cupidatat occaecat ullamco. Deserunt adipisicing laboris nulla nisi. Eiusmod culpa anim commodo magna culpa irure proident amet magna ipsum nulla minim.",
    governmentAuthority: "Oyo Olofin, Nigeria",
    hightestBid: 2.9,
    price: 1.84,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Eiusmod occaecat officia ut minim voluptate voluptate dolore nulla commodo anim. Nisi excepteur laborum pariatur elit culpa ut fugiat labore tempor minim laboris voluptate. Laboris eu consectetur amet laboris amet quis reprehenderit nisi fugiat.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "9/4/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://media.giphy.com/media/QAO5e1GbF9kLKDjqlI/giphy.gif",
      alt: "Art",
    },
    bids: [
      {
        name: "Barbara Summers",
        bidAmount: 2.9,
      },
      {
        name: "Emma Fox",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "583016f7-21dd-58f1-af9e-2234e878588f",
    name: "Sally Williamson",
    verified: "afen",
    description:
      "Proident laboris aliqua proident amet sint non ullamco. Deserunt aliqua incididunt culpa id magna do irure mollit id. Sint aliqua aliquip exercitation ad elit qui sunt commodo cupidatat commodo ad est duis.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2.85,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Ea nostrud enim exercitation dolor. Pariatur enim non veniam reprehenderit sunt. Nisi sunt fugiat consequat adipisicing amet anim eu sunt commodo laborum est qui officia tempor.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "8/9/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://media.giphy.com/media/l3vRg13bg9FdLYK8E/giphy.gif",
      alt: "Art",
    },
    bids: [
      {
        name: "Josephine Munoz",
        bidAmount: 2.9,
      },
      {
        name: "Virgie Nguyen",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "53135294-a208-5a29-9bc3-ff95535c0122",
    name: "Isaac Cole",
    verified: "afen",
    description:
      "Eiusmod laborum non dolore laborum minim fugiat fugiat irure incididunt Lorem. Ad mollit voluptate dolor proident culpa. Consectetur amet esse fugiat excepteur magna.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Anim ullamco adipisicing sit ex aliquip proident nulla ea duis. Et commodo consequat ea ullamco aliqua est voluptate. Excepteur proident exercitation ad esse exercitation.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "1/26/2121",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1515658323406-25d61c141a6e?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Nnx8YWZyaWNhbnxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Andrew Vargas",
        bidAmount: 2.9,
      },
      {
        name: "Zachary Klein",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "881b77a1-eb10-5951-8a33-c9ad1c2c69f0",
    name: "Why are you running!!!",
    verified: "afen",
    description:
      "Anim duis id culpa adipisicing voluptate irure dolor. Aliqua ullamco enim laboris culpa. Ullamco eu occaecat cillum id consequat fugiat sit cillum eu.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 1.59,
    auction: true,
    expires: "",
    owner: {
      name: "Lola",
      description:
        "Sint officia adipisicing quis incididunt exercitation ea cupidatat culpa cupidatat. Duis in mollit incididunt sit laboris pariatur dolor. Aliqua Lorem incididunt consectetur aliquip labore elit ad.",
      profileImage:
        "https://media.giphy.com/media/vwjfBggP3Q5vEdTx9H/giphy.gif",
    },
    created: "1/30/2121",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://media.giphy.com/media/28Nev3k184A9PdSDCg/giphy.gif",
      alt: "Art",
    },
    bids: [
      {
        name: "Wesley Kelly",
        bidAmount: 2.9,
      },
      {
        name: "Harry Goodwin",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "b5c261bd-d145-5749-a81d-083e1c64efa2",
    name: "Fred Davidson",
    verified: "afen",
    description:
      "Cupidatat est laboris mollit ipsum sit dolor exercitation non enim nostrud minim amet. Dolore consectetur laboris esse consequat nisi incididunt. Deserunt eu nulla amet sit do in veniam est veniam Lorem officia minim.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 1.72,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Adipisicing ut amet nostrud do ullamco reprehenderit reprehenderit labore ipsum commodo ullamco minim. Duis id sunt adipisicing excepteur. Duis pariatur do in pariatur aliquip voluptate.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "7/19/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1570158268183-d296b2892211?ixid=MnwxMjA3fDB8MHxzZWFyY2h8N3x8YWZyaWNhbnxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Cynthia McCoy",
        bidAmount: 2.9,
      },
      {
        name: "Edward Rice",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "b41bb00f-13e3-5c21-9a07-8faed005d492",
    name: "Ori Metá",
    verified: "government",
    description:
      "Lorem nulla eu ea cupidatat. Et quis laborum Lorem proident Lorem nostrud dolore. Excepteur duis ad deserunt nostrud ad quis exercitation amet laboris et non commodo do.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2.32,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Tempor non deserunt commodo fugiat tempor duis laboris esse qui proident aliquip est anim id. Sint sit est nostrud occaecat mollit sint occaecat ut reprehenderit laborum in in ex. Qui qui et ex id ea irure ad aute voluptate do fugiat tempor minim sint.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "12/1/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1590796400604-06358f9fe902?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8YWZyaWNhbiUyMGFydHxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Mabelle Quinn",
        bidAmount: 2.9,
      },
      {
        name: "Celia Mendez",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "37437970-5895-5d6e-b236-c718c2d72d98",
    name: "Ayo",
    verified: "government",
    description:
      "Laboris aliquip esse cillum ut ullamco commodo magna labore aliquip eiusmod anim ipsum ut. Incididunt consectetur enim tempor deserunt non ex Lorem. Reprehenderit nisi et adipisicing laborum laborum velit elit est reprehenderit minim.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 1.2,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Ut eiusmod nostrud nostrud magna nostrud et enim. Exercitation dolor elit commodo cupidatat irure sunt do nostrud ea in sint aliqua. Incididunt minim amet ea duis anim exercitation fugiat Lorem minim commodo elit nostrud.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "1/23/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1540247110674-31e928ee852a?ixid=MnwxMjA3fDB8MHxzZWFyY2h8NTB8fGFmcmljYXxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Lela McDaniel",
        bidAmount: 2.9,
      },
      {
        name: "Alice Bowers",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "1e8d1680-e0d9-5706-b55d-08376cbd59ce",
    name: "ọwọ",
    verified: "government",
    description:
      "Nostrud ipsum reprehenderit aliqua est laborum velit sint ea laboris sint ullamco exercitation dolore. Ut consequat cillum anim officia ut cillum elit magna ipsum consequat eiusmod. Nostrud pariatur eu anim aute nisi velit do cupidatat nisi voluptate nisi in anim.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2.07,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Consequat irure irure non elit commodo dolore fugiat. Deserunt eu laboris deserunt sint cillum minim ipsum ad. Aute magna et aute deserunt minim nisi ex in occaecat veniam.",
      profileImage:
        "https://media.giphy.com/media/vwjfBggP3Q5vEdTx9H/giphy.gif",
    },
    created: "1/4/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1501472312651-726afe119ff1?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MzJ8fGFydHxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Mollie Powell",
        bidAmount: 2.9,
      },
      {
        name: "Tom Medina",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "58e1c223-8afc-52c8-a7dd-45ab76baa4ef",
    name: "ipalọlọ afẹfẹ",
    verified: "government",
    description:
      "Consectetur cillum dolore adipisicing ea aliqua occaecat cillum officia. Aliquip adipisicing reprehenderit culpa fugiat fugiat anim aute amet cillum. Adipisicing ipsum duis sunt in exercitation nulla est ex ad aliqua Lorem.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2.84,
    auction: true,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Sint do qui ut veniam pariatur velit non. Culpa incididunt eiusmod ipsum sint. Consectetur sit dolor laboris aliqua minim cillum.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "10/15/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1558591710-4b4a1ae0f04d?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MzB8fGFydHxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Bertie Brooks",
        bidAmount: 2.9,
      },
      {
        name: "Catherine Lyons",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "7711bbb1-71f4-523e-8795-f18a5fe8fbab",
    name: "ọlọrun ti awọn ọjọ",
    bids: 6,
    verified: "government",
    description:
      "Eu quis sint eu ad magna eu do culpa dolore. Laboris officia dolor nostrud quis eu. Laborum officia ipsum sit exercitation id irure.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2.35,
    auction: true,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Minim do nisi voluptate sit sit cupidatat commodo elit cupidatat nulla duis. Amet ullamco nisi et magna est veniam reprehenderit adipisicing duis do voluptate cillum cupidatat deserunt. Velit occaecat reprehenderit culpa ipsum ipsum occaecat proident.",
      profileImage:
        "https://media.giphy.com/media/vwjfBggP3Q5vEdTx9H/giphy.gif",
    },
    created: "2/7/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
      alt: "Art",
    },
    bids: [
      {
        name: "Dennis Hines",
        bidAmount: 2.9,
      },
      {
        name: "Lester Cohen",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "e325ef2a-b154-5b5b-861a-d18ab4f68bf8",
    name: "Louise Riley",
    verified: "government",
    description:
      "Occaecat eu duis quis officia eu ullamco. Proident eiusmod ex aute sunt aute ea minim proident. Ullamco amet reprehenderit veniam labore cupidatat magna id deserunt eu nostrud.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 2.54,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Culpa consequat sit duis magna ullamco nostrud nulla ullamco reprehenderit. Magna pariatur adipisicing cupidatat fugiat commodo eiusmod laborum. Consequat anim nisi consequat laborum esse eiusmod qui minim sunt reprehenderit tempor aliqua Lorem velit.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "4/18/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1561214115-f2f134cc4912?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTB8fGFydHxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Rosa Bass",
        bidAmount: 2.9,
      },
      {
        name: "Ophelia Hicks",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "4cdbf4b7-8ada-51e4-b299-088d1784ae36",
    name: "Martha Weaver",
    verified: null,
    description:
      "Laborum consectetur consequat incididunt adipisicing do proident irure. Occaecat aliqua do cupidatat dolor. Laborum culpa ut ad ullamco est.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 0.46,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Incididunt quis enim reprehenderit occaecat laboris. Consequat aute est cillum adipisicing dolore enim ut. Magna eiusmod irure exercitation laboris magna non dolore esse aliquip voluptate.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "4/4/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://media.giphy.com/media/l2SpPrxbOiKMzR8wo/giphy.gif",
      alt: "Art",
    },
    bids: [
      {
        name: "Ruth Alexander",
        bidAmount: 2.9,
      },
      {
        name: "Lucas Salazar",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "a159c300-a14f-54a8-8290-132996ce2057",
    name: "Madge Lamb",
    verified: null,
    description:
      "Exercitation consequat exercitation aliqua sit duis. Culpa occaecat dolore eiusmod commodo duis exercitation ad pariatur. Non veniam consequat labore qui elit amet ea labore enim elit ut dolore.",
    governmentAuthority: "Nigeria",
    hightestBid: 2.9,
    price: 1.24,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Exercitation mollit officia irure cillum. Qui eu nisi excepteur nulla labore magna commodo proident culpa mollit dolor voluptate sit aliqua. Tempor ipsum minim labore nisi Lorem consequat cupidatat quis proident officia.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "12/21/2121",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1531384441138-2736e62e0919?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MzB8fG5pZ2VyaWF8ZW58MHx8MHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Bradley Simmons",
        bidAmount: 2.9,
      },
      {
        name: "Jayden Waters",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "56354d1b-0f3e-50e2-aa19-6e4eaa96ab44",
    name: "Martha Mathis",
    verified: null,
    description: "basis except",
    hightestBid: 2.9,
    price: 2.36,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Aliquip sint ea non ad. Incididunt officia id officia eiusmod elit consectetur. Aliqua amet fugiat laborum et ut minim ea.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "10/31/2121",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1567536894065-1d8a627a9561?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mzd8fG5pZ2VyaWF8ZW58MHx8MHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Benjamin Pittman",
        bidAmount: 2.9,
      },
      {
        name: "Gilbert Briggs",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "73c38137-0121-5068-8f43-75849be9e7b4",
    name: "David Jefferson",
    description:
      "Nostrud dolor aliqua ullamco ipsum incididunt esse. Deserunt id mollit ipsum pariatur proident do do esse elit elit eu Lorem. Lorem aute deserunt sint incididunt labore consectetur minim.",
    hightestBid: 2.9,
    price: 1.87,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Aliqua proident aute laborum nulla eiusmod Lorem dolore esse sunt nisi sunt aliquip dolor anim. Ex non culpa est ex duis dolor tempor sit Lorem. Minim duis irure exercitation incididunt ipsum nulla eiusmod id officia proident.",
      profileImage:
        "https://media.giphy.com/media/vwjfBggP3Q5vEdTx9H/giphy.gif",
    },
    created: "9/6/2021",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1522168709594-942fcde3d332?ixid=MnwxMjA3fDB8MHxzZWFyY2h8NTd8fGFmcmljYW58ZW58MHx8MHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Lucille Salazar",
        bidAmount: 2.9,
      },
      {
        name: "Steve Cummings",
        bidAmount: 2.18,
      },
    ],
  },
  {
    id: "31a84810-34c0-50fc-962c-9594b2a10103",
    name: "Derrick Robinson",
    description:
      "Proident proident ipsum est aliquip nisi. Aliqua incididunt proident laborum dolore nostrud adipisicing ex eiusmod pariatur anim aliquip anim. Occaecat enim laborum do laborum.",
    hightestBid: 2.9,
    price: 1.52,
    auction: false,
    expires: "",
    owner: {
      name: "Curtis Flores",
      description:
        "Et magna et velit ex dolore enim labore non ex do. Elit incididunt do ex est occaecat culpa laboris. Mollit consequat aute elit ad eu quis tempor et aute aute elit consectetur commodo eiusmod.",
      profileImage:
        "https://media.giphy.com/media/yDe13oXmaBmvuyJaPj/giphy.gif",
    },
    created: "10/13/2121",
    royalty: 10,
    likes: 10,
    image: {
      src: "https://images.unsplash.com/photo-1606094092594-93c09d76b721?ixid=MnwxMjA3fDB8MHxzZWFyY2h8NzB8fGFmcmljYW58ZW58MHx8MHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=60",
      alt: "Art",
    },
    bids: [
      {
        name: "Vincent Allen",
        bidAmount: 2.9,
      },
      {
        name: "Mamie Dunn",
        bidAmount: 2.18,
      },
    ],
  },
];
